...
const CANDIDATES: EditorCandidate[] = [
	...
	},
	{
		...
		commonPaths: [
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text", "sublime_text.exe")),
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text 4", "sublime_text.exe")),
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text 3", "sublime_text.exe")),
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text", "subl.exe")),
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text 4", "subl.exe")),
			...WINDOWS_PROGRAM_FILES.map((root) => join(root, "Sublime Text 3", "subl.exe")),
			"/usr/bin/subl",
			"/usr/local/bin/subl",
		...
		windowsRegistryNames: ["sublime text", "sublime_text"],
	},
	{
		id: "idea",
		name: "IntelliJ IDEA",
